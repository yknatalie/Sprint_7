...
class TestLoginCourier:
    ...
    def test_no_password_error(self, signed_up_courier):
        r = requests.post(urls.MAIN_URL + urls.SIGNED_IN_COURIER,
                          data={'login': signed_up_courier[0]})
        assert r.status_code == 400 and r.text == {"message": "Недостаточно данных для входа"}
    ...
    def test_no_login_error(self, signed_up_courier):
        ...
        assert r.status_code == 400 and r.text == '{"code":400,"message":"Недостаточно данных для входа"}'
    ...
    def test_not_existed_login_error(self):
        ...
        assert r.status_code == 404 and r.text == '{"code":404,"message":"Учетная запись не найдена"}'
